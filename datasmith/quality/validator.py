...
def check_formats(df: pd.DataFrame, schema: list[dict]) -> list[ValidationError]:
    ...
    for col in schema:
        ...
        # ID format check — strict PREFIX-NUMBERS pattern applies only when
        # the schema description declares a format; otherwise IDs just need
        # to be non-empty and unique (UUIDs, lowercase, numeric all valid).
        if re.search(r"\bid\b", desc) or re.search(r"_id$", name, re.I):
            if re.search(r"format|pattern|prefix", desc):
            ...
            elif non_null.duplicated().any():
                dupes = int(non_null.duplicated().sum())
                errors.append(ValidationError(
                    column=name,
                    check="uniqueness_check",
                    message=f"{dupes}/{len(non_null)} duplicate ID values",
                    details={"bad_count": dupes, "total": len(non_null)},
                ))
